namespace Atividade_1
{
    public static class ActionLD
    {
        ...
        //Separar uma lista em duas a partir de uma posição passada
        public static (ListaDupla primeiro, ListaDupla segundo) Separar(ListaDupla l12, int pos)
        {
            ListaDupla l1 = new ListaDupla();
            ListaDupla l2 = new ListaDupla();

            for (int i = 0; i < l12.lenght(); i++)
            {
                if (i < pos)
                {
                    l1.addEnd(l12.GetCelula(i).getElemento());
                }
                else
                {
                    l2.addEnd(l12.GetCelula(i).getElemento());
                }
            }
            return (l1, l2);
        }
